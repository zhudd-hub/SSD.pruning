...
class SlimmingPruner(BasePruner):
    def __init__(self, model, newmodel, args, pruneratio=0.1):
        super().__init__(model, newmodel,  args)
        ...
        for b in self.blocks:
            if b.bnscale is not None:
                ...
                if isinstance(b.bnscale, list):
                    thres_perlayer[b] = [css_thresholding(scale, OT_DISCARD_PERCENT=5e-1) for scale in b.bnscale]
                else:
                    thres_perlayer[b] = css_thresholding(b.bnscale, OT_DISCARD_PERCENT=5e-1)
