...
from model.utils.metric_logger import MetricLogger
    ...
    def upBN(self):
        for m in self.model.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.weight.grad.data.add_(self.args.s * torch.sign(m.weight.data))  # L1
...
def do_train(cfg, model,
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if iteration % args.save_step == 0:
            if args.sr:
                checkpointer.save("sr_model_{:06d}".format(iteration), **arguments)
            else:
                torch.save(model,os.path.join(cfg.OUTPUT_DIR,"fine_pruned_model_{:06d}.pth".format(iteration)))
